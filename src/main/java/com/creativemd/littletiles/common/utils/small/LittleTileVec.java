...
public class LittleTileVec {
    ...
    public LittleTileVec(String name, NBTTagCompound nbt) {
        if (nbt.getTag(name + "x") instanceof NBTTagByte) {
            ...
            writeToNBT(name, nbt);
        } else set(nbt.getInteger(name + "x"), nbt.getInteger(name + "y"), nbt.getInteger(name + "z"));
    }
    ...
    public boolean equals(Object object) {
        if (object instanceof LittleTileVec) return x == ((LittleTileVec) object).x && y == ((LittleTileVec) object).y
                && z == ((LittleTileVec) object).z;
