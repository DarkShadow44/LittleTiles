...
public class GuiContainerSub extends GuiContainer {
    ...
    public void resize() {
        ...
        for (int i = 0; i < layers.size(); i++) {
            if (layers.get(i).width > this.xSize) this.xSize = layers.get(i).width;
            if (layers.get(i).height > this.ySize) this.ySize = layers.get(i).height;
        }
    }
    ...
    public int getMaxScale(Minecraft mc) {
        ...
        while (scaleFactor < k && xSize * (scaleFactor + 1) <= mc.displayWidth
                && ySize * (scaleFactor + 1) <= mc.displayHeight) {
            ++scaleFactor;
        }

        /*
         * if (flag && this.scaleFactor % 2 != 0 && this.scaleFactor != 1) { --this.scaleFactor; }
         */
        return scaleFactor;
    }
    ...
    public SubGui getTopLayer() {
        if (hasTopLayer()) return layers.get(layers.size() - 1);
        return null;
    }

    @Override
    public void keyTyped(char character, int key) {
        if (getTopLayer() != null && !getTopLayer().keyTyped(character, key)) super.keyTyped(character, key);
    }
